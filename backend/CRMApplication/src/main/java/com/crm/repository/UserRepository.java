...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT new com.crm.dto.UserInteractionCountDTO(u.username, COUNT(i.id)) " +
           "FROM User u JOIN Interaction i ON u.id = i.customer.id " +
           "WHERE u.role = 'ROLE_CUSTOMER' " +
           "GROUP BY u.username ORDER BY COUNT(i.id) DESC")
    List<UserInteractionCountDTO> findTopCustomersByInteractions(Pageable pageable);
}